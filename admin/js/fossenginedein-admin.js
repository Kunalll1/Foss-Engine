(function ($) {
  ...
  $(document).ready(function () {
    ...
    $("#csv-upload-form").on("submit", function (e) {
      ...
      $.ajax({
        ...
        error: function () {
          ...
          $("#upload-results")
            .html(
              '<div class="notice notice-error"><p>An unexpected error occurred. Please try again.</p></div>'
            )
            .show();
        },
      });
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            ...
            rowElement
              .find(".column-status .status-badge")
            ...
            var newButtons =
              '<button class="button button-primary edit-content-button" data-id="' +
              topicId +
              '">Edit</button> ' +
              '<button class="button regenerate-content-button" data-id="' +
              topicId +
              '">Regenerate</button>';
    ...
    $(".topics-table").on("click", ".edit-content-button", function () {
      ...
      var topicTitle = $("#topic-row-" + topicId + " .column-topic")
        .text()
        .trim();
      ...
      if (
        ...
        tinymce.get("topic-content-editor")
      ) {
        tinymce
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            ...
            if (
              ...
              tinymce
                .get("topic-content-editor")
                .setContent("<p>Error: " + response.data.message + "</p>");
            } else {
        ...
        error: function (xhr, status, error) {
          ...
          if (
            ...
            tinymce
              .get("topic-content-editor")
              .setContent("<p>Error Details:</p><pre>" + errorMsg + "</pre>");
          } else {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.success) {
            ...
            rowElement
              .find(".column-actions")
              .html('<span class="published-status">Content Published</span>');
    ...
    $("#generate-all-button").on("click", function () {
      ...
      if (
        !confirm(
          "Are you sure you want to generate content for all " +
            pendingButtons.length +
            " pending topics? This might take some time."
        )
      ) {
        return;
      }
      ...
      $(this)
        .prop("disabled", true)
        .html(
          '<span class="spinner is-active" style="float: none; margin-top: 0;"></span> Generating all...'
        );
